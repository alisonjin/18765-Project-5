...
def read_etr(infetrname):
    ...
    for line in infile: #reads file line-by-line
        ...
        if cksig:
            print('Error: Circuit used to create \'.etr\' file should be purely combinational for serialization\n')
            sys.exit(2)
    ...
    for sig in scanpairdict.keys():
        for i in scanchains:
            if sig in scanchains[i] and scanpairdict[sig] in scanchains[i]:
                print('Error: both signals of scan pair cannot be present in scan chains')
...
def write_etr(clocks, indict, outdict, scanchains, controls, pins, pouts,
    ...
    outf.write(str(len(list(indict.values())[0])+1) + ' vectors in file\n')
    ...
    outf.write('** Inputs must have value \'0\', \'1\', or \'X\'\n')
    outf.write('** Outputs must have value \'0\', \'1\', or \'X\'\n\n\n')
    ...
    for i in range(len(indict[list(indict.keys())[0]])+1):
        #scanchain vectors
...
def makechaindict(indict, outdict, scanchains, scanpairdict, veclen,
    ...
    for chain in scanchains:
        ...
        for i  in range(len(indict[list(indict.keys())[0]])):
...
def main():
    ...
    print("{} input signals found".format(len(innames)))
    ...
    print("{} primary inputs defined".format(len(pins)))
    print("{} primary outputs defined".format(len(pouts)))
    print("{} boundary scan signals defined".format(len(fins) + len(fouts)))
    print("{} internal scan flip flops defined".format(len(scanpairdict)))
    print("{} clock signals defined".format(len(clocks)))
    print("{} control signals defined".format(len(controls)))
    print("{} scanchains defined".format(len(scanchains)))
    for chain in scanchains:
        print("\tScanchain {} of length {}".format(len(clocks), len(scanchains[chain])))
    print('\n')
    ...
    print("Vector length: {}".format(veclen))
